...
SourceKit-LSP, Kotlin LSP, Java LSP, and similar semantic tools can prove semantic relationships and provide diagnostics. They do not prove:

- the app launches;
- a screen behaves correctly;
- a simulator/emulator flow works;
- tests pass;
- build settings are fresh.

Use Xcode, Android Studio, Gradle, CI, or the build system for that.
...
- a reference points to the intended Kotlin or Java symbol;
- a protocol conformance is real;
- an interface implementation or override is real;
...
Generated files, XIB/storyboard files, Android XML resources, localization, GraphQL schemas, and resource surfaces often require discovery-first handling:
...
```text
fd/rg -> GraphQL/resource/manifest validation when applicable -> LSP only if mapped to Kotlin/Java source -> Gradle/emulator/device proof if behavior matters
```
