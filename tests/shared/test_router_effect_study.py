...
class RouterEffectStudyTests(unittest.TestCase):
    ...
    def test_hermetic_codex_config_diff_limits_a_to_d_to_treatment_fields(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("high_fanout_policy", diff["required_fields"])
            self.assertIn("high_fanout_policy", diff["changed_fields"])

            collapsed = dict(env_d.effective_config)
            collapsed["high_fanout_policy"] = env_a.effective_config["high_fanout_policy"]
            collapsed_diff = diff_effective_agent_configs(
                env_a.effective_config,
                collapsed,
                left_profile_id="A-search-only",
                right_profile_id="D-full-router",
            )
            self.assertFalse(collapsed_diff["valid"], collapsed_diff)
            self.assertIn("high_fanout_policy", collapsed_diff["missing_required_fields"])
