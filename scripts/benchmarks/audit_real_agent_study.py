    ...
    *,
    ...
    for key, profile_rows in cells_by_block.items():
        ...
        for left, right in FACTORIAL_COMPARISONS:
            ...
            if not diff["valid"]:
                details = []
                if diff.get("disallowed_fields"):
                    details.append("disallowed=" + ",".join(str(item) for item in diff["disallowed_fields"]))
                if diff.get("missing_required_fields"):
                    details.append("missing_required=" + ",".join(str(item) for item in diff["missing_required_fields"]))
                add_issue(
                    issues,
                    "fail",
                    "treatment_diff",
                    f"{key} {left} vs {right} violates treatment diff allowlist: {'; '.join(details)}",
                )
